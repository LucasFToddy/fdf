...
{
	coords->z = data->z_matriz[
		(int)(coords->y * data->width + coords->x)].value;
	coords->z1 = data->z_matriz[
		(int)(coords->y1 * data->width + coords->x1)].value;
	coords->start_color = ft_atoi_hexa(data->z_matriz[
			(int)(coords->y * data->width + coords->x)].color);
}
...
int main(int argc, char **argv)
{
	...
	{
		make_img(&data);
		mlx_key_hook(data.data->win_ptr, key_hook, &data);
