...
static void	set_pixel(t_coords *coords, t_fdf_gen *data)
{
	...
	index = 0;
	if (coords->x >= 0 && coords->x < data->data->win_width &&
		coords->y >= 0 && coords->y < data->data->win_height)
	{
		index = ((int)(coords->y) * data->img_data->size_line + (int)(coords->x) * (data->img_data->bpp / 8));
